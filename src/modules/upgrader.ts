...
import {getHeroInfo, improveSkill} from "../api/muskempire/musk-empire-api.js";
...
export const upgrader = async (account: MuskEmpireAccount, apiKey: string) => {
  ...
  const {data: {data: heroInfo}} = await getHeroInfo(apiKey)
  const bestUpgrade = upgrades
	.filter((upgrade) => upgrade.priceNextLevel <= heroInfo.money)
	.reduce((best, upgrade) =>
		best === null || (upgrade.profitIncrement / upgrade.priceNextLevel) > (best.profitIncrement / best.priceNextLevel) ? upgrade : best,
